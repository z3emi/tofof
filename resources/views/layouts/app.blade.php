              ...
              <div class="flex w-full items-center justify-between min-h-[42px] relative gap-4">
                ...
                <div class="flex items-center justify-end gap-1 sm:gap-2.5 z-10">
                  ...
                  <div class="relative" x-data="userNotificationsComponent('{{ route('user.notifications.index') }}', '{{ route('user.notifications.markAsRead') }}')" x-init="fetchNotifications(); setInterval(() => fetchNotifications(), 60000)" x-ref="notificationContainerMobile">
                    ...
                    <template x-teleport="body">
                            <div x-show="dropdownOpen" @click.away="dropdownOpen = false" class="text-right notification-popup fixed z-[9999] bg-white dark:bg-gray-800 shadow-2xl rounded-xl border border-gray-100" style="display:none;"
                              x-init="$watch('dropdownOpen', (value) => { if (value) { $nextTick(() => { let rect = $refs.notificationContainerMobile.getBoundingClientRect(); const horizontalPadding = 10; const maxPopupWidth = 384; const availableWidth = Math.max(260, window.innerWidth - (horizontalPadding * 2)); const popupWidth = Math.min(maxPopupWidth, availableWidth); $el.style.width = `${popupWidth}px`; $el.style.maxWidth = `${popupWidth}px`; $el.style.top = `${rect.bottom + 12}px`; let safeLeft = Math.max(horizontalPadding, Math.floor((window.innerWidth - popupWidth) / 2)); if (safeLeft + popupWidth > window.innerWidth - horizontalPadding) safeLeft = window.innerWidth - popupWidth - horizontalPadding; $el.style.left = `${Math.max(horizontalPadding, safeLeft)}px`; $el.style.right = 'auto'; }); } })">
                          <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 font-bold text-gray-800 dark:text-gray-100">{{ __('layout.notifications') }}</div>
