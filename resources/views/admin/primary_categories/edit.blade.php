...
<div class="form-card">
    <div class="form-card-header d-flex justify-content-between align-items-center">
        <div>
            <h2 class="mb-2 fw-bold text-white"><i class="bi bi-pencil-square me-2"></i> تعديل الفئة: {{ $item->name_ar }}</h2>
            <p class="mb-0 opacity-75 fs-6 text-white small">أنت الآن تقوم بتحديث بيانات الفئة المختارة.</p>
        </div>
    ...
    <div class="p-4 p-lg-5">
        ...
        <form action="{{ route('admin.primary-categories.update', $item->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mt-5 pt-4 border-top d-flex gap-3 justify-content-end">
                <a href="{{ route('admin.primary-categories.index') }}" class="btn btn-light px-4 py-2 rounded-3">إلغاء</a>
                <button type="submit" class="submit-btn shadow-sm">
                    <i class="bi bi-cloud-upload me-1"></i> تحديث الفئة
                </button>
            </div>
        </form>
    </div>
