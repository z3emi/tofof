...
<div class="card shadow-sm">
  <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
    <h4 class="mb-0 text-danger"><i class="bi bi-trash"></i> سلة محذوفات الفئات</h4>
    <a href="{{ route('admin.primary-categories.index') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <tbody>
          @forelse ($items as $item)
            <tr>
              ...
              <td>
                @if($item->image)
                  <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->name_ar }}" class="cat-thumb">
                @else
                  <div class="thumb-fallback text-muted">No Img</div>
                @endif
              </td>
              <td class="text-start fw-bold">{{ $item->name_ar }}</td>
              <td>{{ $item->deleted_at->format('Y-m-d H:i') }}</td>
              <td>
                <form action="{{ route('admin.primary-categories.restore', $item->id) }}" method="POST" class="d-inline-block">
                  @csrf
                ...
                <form action="{{ route('admin.primary-categories.forceDelete', $item->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('تنبيه: سيتم مسح الفئة نهائياً ولا يمكن التراجع. هل أنت متأكد؟')">
                  @csrf @method('DELETE')
