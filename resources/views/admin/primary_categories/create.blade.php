...
@section('title', 'إضافة فئة منتجات جديدة')
...
<div class="form-card">
    <div class="form-card-header d-flex justify-content-between align-items-center">
        <div>
            <h2 class="mb-2 fw-bold text-white"><i class="bi bi-folder-plus-fill me-2"></i> إضافة فئة جديدة</h2>
            <p class="mb-0 opacity-75 fs-6 text-white small">قم بتعريف فئة جديدة لتنظيم المنتجات بشكل أفضل.</p>
        </div>
        <a href="{{ route('admin.primary-categories.index') }}" class="btn btn-outline-light px-4 fw-bold rounded-3">
            <i class="bi bi-arrow-right me-1"></i> العودة للفئات
        </a>
    </div>
    ...
    <div class="p-4 p-lg-5">
        ...
        <form action="{{ route('admin.primary-categories.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                <h5 class="form-section-title">بيانات الفئة</h5>
            ...
            <div class="mt-5 pt-4 border-top d-flex gap-3 justify-content-end">
                <a href="{{ route('admin.primary-categories.index') }}" class="btn btn-light btn-cancel">إلغاء</a>
                <button type="submit" class="submit-btn shadow-sm">
                    <i class="bi bi-check2-circle me-1"></i> حفظ الفئة الجديدة
                </button>
            </div>
        </form>
    </div>
