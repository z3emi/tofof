...
<style>
    .form-card {
        border-radius: 0 !important; border: none !important; box-shadow: none !important; background: #fff; width: 100% !important; margin: 0 !important;
    }
    .form-card-header {
        background: linear-gradient(135deg, var(--primary-dark) 0%, var(--primary-medium) 100%);
        padding: 2.5rem 3rem; color: white; border-radius: 0 !important;
    }
    .form-section-title {
        font-weight: 700; color: var(--primary-dark); border-right: 4px solid var(--accent-gold); padding-right: 15px; margin-bottom: 2rem;
    }
    .permission-group-card {
        border: 1px solid #f1f5f9; border-radius: 15px; padding: 1.5rem; background: #fafbff; height: 100%; transition: all 0.3s ease;
    }
    .permission-item {
        background: #fff; border: 1px solid #f1f5f9; border-radius: 12px; padding: 0.8rem 1.2rem; margin-bottom: 0.8rem; display: flex; justify-content: space-between; align-items: center; transition: all 0.2s ease;
    }
    .permission-item:hover { border-color: var(--primary-light); background: #fff9f9; }
...
<div class="form-card">
    <div class="form-card-header d-flex justify-content-between align-items-center">
        <div>
            <h2 class="mb-2 fw-bold text-white"><i class="bi bi-shield-lock-fill me-2"></i> إنشاء دور وظيفي جديد</h2>
            <p class="mb-0 opacity-75 fs-6 text-white small">حدد اسم الدور وخصص الصلاحيات المناسبة لكل قسم.</p>
        </div>
        <div>
            <span class="badge bg-white text-dark py-2 px-4 rounded-pill fw-bold shadow-sm" id="perm-count">0 صلاحيات مختارة</span>
        </div>
    </div>
    ...
    <div class="p-4 p-lg-5">
        <form action="{{ route('admin.roles.store') }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" style="border-radius:12px; padding:0.8rem" id="name" name="name" value="{{ old('name') }}" placeholder="مثال: support-team" required>
            ...
            <div class="d-flex justify-content-end gap-3 pt-5 border-top">
                ...
                <button type="submit" class="submit-btn shadow-sm py-3 px-5">إنشاء وحفظ الدور</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const update = () => countDisplay.textContent = `${document.querySelectorAll('.permission-checkbox:checked').length} صلاحيات مختارة`;
...
<div class="p-5 text-center">
    <div class="alert alert-danger rounded-4 shadow-sm p-5 d-inline-block">
        ...
        <h4>ليس لديك صلاحية لإدارة الأدوار.</h4>
    </div>
</div>
@endcan
@endsection
