...
<div class="top-action-bar">
    <div class="d-flex align-items-center gap-3">
        <div class="role-badge">دور جديد</div>
        <div class="small opacity-75">إضافة مسمى وظيفي جديد وتخصيص صلاحياته</div>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('admin.roles.index') }}" class="btn btn-sm btn-outline-light border-0 opacity-75 px-3">إلغاء</a>
        <button form="permissions-form" type="submit" class="btn btn-save-fixed">إنشاء الدور</button>
    </div>
</div>

<form action="{{ route('admin.roles.store') }}" method="POST" id="permissions-form">
    @csrf
    
    <div class="bg-white p-3 rounded-3 mb-3 border d-flex align-items-center gap-3" dir="rtl">
        <label for="name" class="fw-bold small text-muted text-nowrap">اسم الدور البرمجي:</label>
        <input type="text" class="form-control border-0 bg-light rounded-pill px-4" 
               id="name" name="name" value="{{ old('name') }}" placeholder="e.g., manager, account-editor" required>
    </div>
        ...
        <div class="settings-content">
            <div class="content-header">
                ...
                <div class="d-flex align-items-center gap-2">
                    <span class="small text-muted fw-bold" id="enabled-count">0 مفعلة</span>
...
@else
    <div class="alert alert-danger p-4 rounded-4 shadow-sm text-center">
        ...
        <p class="text-muted">يرجى التواصل مع مدير النظام للحصول على صلاحية إضافة الأدوار الوظيفية.</p>
    </div>
@endcan
@endsection
