...
<style>
  ...
  .thumb-fallback{
    ...
    border:1px solid #eee; color:var(--brand);
  }
  ...
  .children-wrap{
    ...
    margin-right:1.25rem; padding-right:1rem; margin-top:.75rem;
  }
  ...
  /* إصلاح سهم الـ select في RTL (بدون تغيرات خطوط) */
...
<div class="card shadow-sm">
  ...
  <div class="card-body">
    {{-- فلاتر البحث --}}
    <form method="GET" action="{{ route('admin.categories.index') }}" class="row g-2 filters-bar">
      <div class="col">
        <input type="text" name="q" class="form-control" placeholder="ابحث باسم القسم..." value="{{ request('q') }}">
      </div>
      <div class="col-auto">
    ...
    <div class="category-tree" x-data>
      @forelse ($categories as $category)
        <div class="mb-3 tree-branch" x-data="{ open: false }">
          <div class="cat-card parent-category">
            <div class="cat-info">
              @php $img = $category->image ? asset('storage/' . $category->image) : null; @endphp
              @if($img)
                <img src="{{ $img }}" alt="{{ $category->name_ar }}" class="cat-thumb">
              @else
              ...
              <div>
                ...
                <div class="chips">
                  ...
                  <span class="chip">ID: {{ $category->id }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="actions">
              @if($category->children->isNotEmpty())
                <button class="toggle-btn" @click="open = !open" :aria-expanded="open.toString()" title="عرض/إخفاء الأقسام الفرعية">
                  <i class="bi bi-chevron-down toggle-icon"></i>
                </button>
              @endif

              @can('edit-categories')
              <a href="{{ route('admin.categories.edit', $category->id) }}" class="btn btn-sm btn-outline-primary btn-icon" data-bs-toggle="tooltip" title="تعديل">
      ...
      @empty
        <div class="text-center py-4">
          <p class="mb-0 text-muted">لا يوجد أقسام رئيسية لعرضها.</p>
        </div>
      @endforelse
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap gap-2">
      <form method="GET" action="{{ route('admin.categories.index') }}" class="d-flex align-items-center">
        @foreach(request()->except(['per_page', 'page']) as $key => $value)
          <input type="hidden" name="{{ $key }}" value="{{ $value }}">
        @endforeach
        <label for="per_page" class="me-2">عدد الأقسام:</label>
