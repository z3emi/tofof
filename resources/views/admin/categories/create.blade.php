...
<style>
    ...
    .form-control, .form-select { border-radius: 10px; padding: 0.8rem 1.2rem; border: 1px solid #e2e8f0; transition: all 0.2s; }
    .form-control:focus, .form-select:focus { border-color: var(--primary-light); box-shadow: 0 0 0 3px rgba(195, 33, 38, 0.1); }
    ...
    .btn-cancel { padding: 1rem 2.5rem; border-radius: 10px; font-weight: 600; }
</style>
@endpush
...
<div class="form-card">
    <div class="form-card-header d-flex justify-content-between align-items-center">
        <div>
            <h2 class="mb-2 fw-bold text-white"><i class="bi bi-patch-plus-fill me-2"></i> إضافة براند جديد</h2>
            <p class="mb-0 opacity-75 fs-6 text-white small">قم بتعريف العلامة التجارية الجديدة لعرضها في المتجر.</p>
        </div>
    ...
    <div class="p-4 p-lg-5">
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                ...
                @include('admin.categories._form', ['item' => new \App\Models\Category(), 'parentCategories' => $parentCategories])
            </div>

            <div class="d-flex justify-content-end gap-3 pt-5 border-top">
                <a href="{{ route('admin.categories.index') }}" class="btn btn-light px-4 py-2 btn-cancel">إلغاء</a>
                <button type="submit" class="submit-btn shadow-sm">
                    <i class="bi bi-check2-circle me-1"></i> حفظ البراند الجديد
                </button>
            </div>
        </form>
    </div>
