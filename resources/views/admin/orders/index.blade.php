...
<style>
    ...
    .order-row { transition: background-color .2s ease-in-out; }
    .order-row.status-pending > td { background-color: #fff8db !important; }
    .order-row.status-processing > td { background-color: #e8f8ff !important; }
    .order-row.status-shipped > td { background-color: #eef4ff !important; }
    .order-row.status-delivered > td { background-color: #eaf8ee !important; }
    .order-row.status-cancelled > td { background-color: #f2f3f5 !important; }
    .order-row.status-returned > td { background-color: #ffebee !important; }
    .order-row:hover > td {
        box-shadow: inset 0 0 0 9999px rgba(0, 0, 0, 0.05) !important;
    }
    .order-row.is-editing > td {
        box-shadow: inset 0 0 0 9999px rgba(255, 193, 7, 0.22) !important;
    }
    .btn-delete-disabled {
            ...
            <table class="table mb-0 align-middle text-center" id="orders_table">
                ...
                <tbody>
                    @forelse($orders as $order)
                        <tr class="order-row status-{{ $order->status }}" data-order-id="{{ $order->id }}">
                                ...
                                <div class="btn-group">
                                    ...
                                    @can('delete-orders')
                                        @if(in_array($order->status, ['cancelled', 'returned']))
                                            <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST" class="d-inline-block js-delete-order-form" onsubmit="return confirm('هل أنت متأكد من نقل الطلب إلى سلة المحذوفات؟');">
                                                @csrf
                                                @method('DELETE')
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const scrollKey = 'admin_orders_index_scroll_y';
        ...
        const deleteForms = document.querySelectorAll('.js-delete-order-form');
        ...
        statusModal.addEventListener('show.bs.modal', function (event) {
            ...
            currentEditingRow = document.querySelector('.order-row[data-order-id="' + orderId + '"]');
            if (currentEditingRow) {
