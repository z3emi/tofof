...
@section('title', __('إنشاء سند قبض'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3">{{ __('إنشاء سند قبض') }}</h1>
    <a href="{{ route('admin.finance.receipt-vouchers.index') }}" class="btn btn-outline-secondary">{{ __('العودة') }}</a>
</div>

<form method="POST" action="{{ route('admin.finance.receipt-vouchers.store') }}" class="card">
    @csrf
    <div class="card-body row g-3">
        <div class="col-md-4">
            <label class="form-label">{{ __('الرقم المرجعي') }}</label>
            <input type="text" name="number" value="{{ old('number') }}" class="form-control" placeholder="مثال: RV-000123">
        ...
        <div class="col-md-4">
            ...
            <select name="cash_box_id" class="form-select">
                ...
                @foreach($cashBoxes as $cashBox)
                    <option value="{{ $cashBox->id }}" @selected(old('cash_box_id') == $cashBox->id)>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="transaction_channel" value="{{ old('transaction_channel') }}" class="form-control" placeholder="مثال: نقدي / تحويل بنكي">
    ...
    <div class="card-footer d-flex justify-content-end gap-2">
        ...
        <button type="submit" class="btn btn-primary">{{ __('حفظ السند') }}</button>
    </div>
</form>
@endsection
