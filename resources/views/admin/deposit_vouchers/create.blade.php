...
@section('title', __('تسجيل إيداع مندوب'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3">{{ __('تسجيل إيداع مندوب') }}</h1>
    <a href="{{ route('admin.finance.deposit-vouchers.index') }}" class="btn btn-outline-secondary">{{ __('العودة') }}</a>
</div>

<form method="POST" action="{{ route('admin.finance.deposit-vouchers.store') }}" class="card">
    @csrf
    <div class="card-body row g-3">
        <div class="col-md-4">
            <label class="form-label">{{ __('الرقم المرجعي') }}</label>
            <input type="text" name="number" value="{{ old('number') }}" class="form-control" placeholder="مثال: DV-000123">
    ...
    <div class="card-footer d-flex justify-content-end gap-2">
        ...
        <button type="submit" class="btn btn-primary">{{ __('تسجيل الإيداع') }}</button>
    </div>
</form>
@endsection
