...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">إنشاء أمر تصنيع جديد</h4>
...
<div class="card shadow-sm">
    <div class="card-body">
        <form action="{{ route('admin.manufacturing.orders.store') }}" method="POST" id="manufacturing-order-form">
            @csrf
            <div class="row g-3">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <select id="bom-picker" class="form-select">
                        ...
                        @foreach($boms as $bom)
                            <option value="{{ $bom->id }}" data-product="{{ $bom->product_id }}">{{ $bom->product?->name_ar }} @if($bom->variant_name) - {{ $bom->variant_name }} @endif</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label">اسم المتغير</label>
                    <input type="text" name="variant_name" id="variant-name-input" class="form-control" value="{{ old('variant_name') }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label">الكمية المخطط لها <span class="text-danger">*</span></label>
                    <input type="number" min="1" name="planned_quantity" class="form-control" value="{{ old('planned_quantity', 1) }}" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">الكمية المكتملة</label>
                    <input type="number" min="0" name="completed_quantity" class="form-control" value="{{ old('completed_quantity', 0) }}">
                </div>
                <div class="col-md-3">
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle" id="order-materials-table">
                    ...
                    <tbody>
                        @php $oldMaterials = old('materials', []); @endphp
                                ...
                                <td>
                                    <select name="materials[{{ $index }}][material_id]" class="form-select" required>
                                        ...
                                        @foreach($materials as $option)
                                            <option value="{{ $option->id }}" @selected($material['material_id'] == $option->id)>{{ $option->name }} ({{ $option->unit ?? 'وحدة' }})</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    bomPicker.addEventListener('change', event => {
        ...
        if (data.variant_name && !variantInput.value) {
