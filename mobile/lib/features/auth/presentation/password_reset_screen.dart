...
class _PasswordResetScreenState extends ConsumerState<PasswordResetScreen> {
  ...
  Future<void> _sendOtp() async {
    ...
    final success = await ref
        ...
        .requestPasswordResetOtp(phoneNumber: _fullPhone());
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(ref.read(authProvider).error ?? 'تعذر إرسال الرمز'),
      ),
    );
  }
  ...
  Future<void> _resetPassword() async {
    if (_pendingPhone == null || _otpCtrl.text.trim().length != 6) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('أدخل الرمز بشكل صحيح')));
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(ref.read(authProvider).error ?? 'تعذر إعادة التعيين'),
      ),
    );
  }
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    _otpStep
                        ? 'أدخل الرمز الجديد وكلمة المرور الجديدة.'
                        : 'أدخل رقم الهاتف وسيصلك رمز التحقق عبر واتساب.',
                    textAlign: TextAlign.center,
                  ...
                  ElevatedButton(
                    onPressed: authState.isLoading
                        ? null
                        : (_otpStep ? _resetPassword : _sendOtp),
                    child: authState.isLoading
                        ...
                        : Text(
                            _otpStep ? 'تحديث كلمة المرور' : 'إرسال رمز التحقق',
                          ),
                  ),
                  if (_otpStep) ...[
                    const SizedBox(height: 8),
